    ...
    return(
        ...
        <div className="container px-5 py-24 mx-auto">
            <div className="flex flex-col text-center w-full mb-12">
                <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">Login as a Retailer</h1>
                <p className="lg:w-2/3 mx-auto leading-relaxed text-base">Make sure that you are registered user if not,
                    kindly
                    make an account first</p>
            </div>
            <div className="container px-5 py-20 mx-auto flex flex-wrap items-center lg:w-2/6 md:w-1/3 bg-gray-100 rounded-lg flex flex-col">
                <div className="relative flex-grow w-full">
                    <label htmlFor="r_email" className="leading-7 text-sm text-gray-600">Email</label>
                    <input type="email" id="r_email" name="r_email" onChange={Ret_EmailHandler} className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
                </div>
                <div className="relative flex-grow w-full">
                    <label htmlFor="r_password" className="leading-7 text-sm text-gray-600">Password</label>
                    <input type="password" id="r_password" name="r_password" onChange={Ret_PasswordHandler} className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
                </div>
                <button id="r_submit" onClick={Ret_SignUp} className="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg mt-10 justify-center items-center">Submit</button>
            </div>
        </div>
    </section>
        </>
